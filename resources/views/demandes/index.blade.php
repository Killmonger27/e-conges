                        ...
                        <table class="table table-striped" id="table1">
                            ...
                            <tbody>
                                @foreach ($demandes as $demande)
                                    <tr>
                                        <td>{{ $demande->employe->nom ?? 'Non défini' }}
                                            {{ $demande->employe->prenom ?? 'Non défini' }}</td>
                                        <td>
                                            <span
                                                class="badge bg-primary">{{ ucfirst($demande->type_de_demande) }}</span>
                                        </td>
                                        ...
                                        <td class="d-md-flex justify-content-lg-between ">
                                            ...
                                            @if ($demande->status === 'encours' && $user->can('process', $demande))
                                                <form action="{{ route('demandes.approve', $demande) }}" method="post"
                                                    class="d-inline">
                                                    @csrf
                                                    <button type="submit" class="btn btn-success">Approuver</button>
                                                </form>
                                                <form action="{{ route('demandes.reject', $demande) }}" method="post"
                                                    class="d-inline">
                                                    @csrf
                            ...
                            <tbody>
                                @foreach ($demandesService as $demande)
                                    <tr>
                                        ...
                                        <td>{{ Str::limit($demande->motif, 50) }}</td>
                                        <td>{{ \Carbon\Carbon::parse($demande->date_de_demande)->format('d/m/Y') }}
                                        </td>
                                        <td>
                                            {{ \Carbon\Carbon::parse($demande->date_de_debut)->format('d/m/Y') }} -
                                            {{ \Carbon\Carbon::parse($demande->date_de_fin)->format('d/m/Y') }}
